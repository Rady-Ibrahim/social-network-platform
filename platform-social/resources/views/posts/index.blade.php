    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-4 sm:p-6 bg-white shadow sm:rounded-lg">
                <form id="post-form" action="{{ route('posts.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                ...
                @forelse ($posts as $post)
                    <article class="p-4 sm:p-6 bg-white shadow sm:rounded-lg">
                        ...
                        <div class="mt-3 flex items-center gap-4 flex-wrap">
                            <form action="{{ $post->is_liked_by_me ? route('posts.unlike', $post) : route('posts.like', $post) }}" method="post" class="inline">
                                @csrf
                                @if ($post->is_liked_by_me)
                                    @method('DELETE')
                                @endif
                                <button type="submit" class="text-sm font-medium {{ $post->is_liked_by_me ? 'text-indigo-600' : 'text-gray-600' }} hover:underline">
                                    {{ $post->is_liked_by_me ? __('Unlike') : __('Like') }}
                                </button>
                            </form>
                            <a href="{{ route('posts.show', $post) }}" class="text-sm text-gray-500 hover:underline">
                                {{ $post->likes_count }} {{ __('likes') }}
                            </a>
